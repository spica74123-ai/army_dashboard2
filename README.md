คู่มือการติดตั้งระบบบัญชาการ ฝกพ.ศปก.ทบ.

ระบบนี้พัฒนาด้วยสถาปัตยกรรม MERN Stack (MongoDB, Express.js, React/Next.js, Node.js)

📋 สิ่งที่ต้องเตรียม (Prerequisites)

ก่อนเริ่มการติดตั้ง กรุณาตรวจสอบให้แน่ใจว่าเครื่องคอมพิวเตอร์ของคุณมีโปรแกรมเหล่านี้:

Node.js (แนะนำเวอร์ชัน 18.x ขึ้นไป) - ดาวน์โหลด Node.js

MongoDB - สามารถใช้เวอร์ชันติดตั้งในเครื่อง (MongoDB Community Server) หรือใช้บนคลาวด์ผ่าน MongoDB Atlas

Code Editor - แนะนำ Visual Studio Code (VS Code)

🛠️ ส่วนที่ 1: การติดตั้ง Backend (Node.js + Express + MongoDB)

สร้างโฟลเดอร์สำหรับ Backend
...
สร้างไฟล์สำหรับเซิร์ฟเวอร์

สร้างไฟล์ชื่อ server.js แล้วคัดลอกโค้ด Backend ทั้งหมด (จากไฟล์ server.js ที่ผมให้ไป) มาวาง

สร้างไฟล์ชื่อ .env ในโฟลเดอร์เดียวกัน เพื่อตั้งค่าตัวแปรระบบ ดังนี้:

PORT=3001
MONGODB_URI=mongodb://localhost:27017/rta_tactical_db


(หากคุณใช้ MongoDB Atlas ให้นำ Connection String มาใส่แทน mongodb://localhost...)
...
หากสำเร็จ จะขึ้นข้อความว่า: ✅ เชื่อมต่อ MongoDB สำเร็จ! และ 🚀 Server เปิดทำงานที่พอร์ต http://localhost:3001
(ระบบจะสร้าง User Admin เริ่มต้นให้โดยอัตโนมัติ: admin / รหัสผ่าน 123)

💻 ส่วนที่ 2: การติดตั้ง Frontend (Next.js + Tailwind CSS)

สร้างโปรเจกต์ Next.js
เปิด Terminal ใหม่ (อย่ายกเลิกการทำงานของ Backend) นอกโฟลเดอร์ rta-backend แล้วรันคำสั่ง:

npx create-next-app@latest rta-frontend


เมื่อระบบถามการตั้งค่า ให้เลือกตามนี้:

Would you like to use TypeScript? No

Would you like to use ESLint? No

Would you like to use Tailwind CSS? Yes (สำคัญมาก)
...
ติดตั้งไลบรารีเพิ่มเติม (แม้ว่าโค้ดส่วนใหญ่จะใช้ CDN ผ่าน useEffect แต่แนะนำให้ติดตั้งไว้เผื่อการต่อยอด)
...
ไปที่ไฟล์ app/page.js (ลบโค้ดเดิมทิ้งทั้งหมด)

นำโค้ด Frontend (React/JSX ที่เป็น App.jsx ที่ผมเขียนให้) มาวางทับลงไป

จุดสำคัญ: ในโค้ด page.js ให้ค้นหาตัวแปร GAS_API_URL แล้วเปลี่ยนให้ชี้มาที่ Backend ของคุณ:

// เปลี่ยนจาก URL ของ Google Script เป็น:
...
🚀 ส่วนที่ 3: การรันระบบ

ก่อนรันระบบ กรุณาตรวจสอบว่า MongoDB ทำงานอยู่

เปิด Terminal ใหม่ แล้วรันคำสั่ง:

mongod

(หากใช้ MongoDB เป็น service สามารถข้ามได้)
...
เปิดเบราว์เซอร์ไปที่ http://localhost:3000 เพื่อเข้าสู่แดชบอร์ด

หรือเปิด index.html ในโฟลเดอร์หลักเพื่อเข้าหน้า landing

👤 บัญชี Admin เริ่มต้น: username: admin, password: 123

เปิดเบราว์เซอร์ไปที่ http://localhost:3000 (พอร์ตเริ่มต้นของ Next.js)

หน้าจอ Login จะแสดงขึ้นมา

ใช้บัญชีผู้ดูแลระบบที่ Backend สร้างให้เพื่อเข้าสู่ระบบ:

ชื่อผู้ใช้งาน: admin

รหัสผ่าน: 123

เมื่อเข้าสู่ระบบสำเร็จ คุณสามารถทดลองเพิ่ม/ลบ/แก้ไข โครงสร้างหน่วย และกำลังพลได้เลย ข้อมูลทั้งหมดจะถูกบันทึกลงใน MongoDB

☁️ คำแนะนำสำหรับการนำขึ้นระบบจริง (Deployment)

เมื่อพัฒนาเสร็จและต้องการนำระบบให้ผู้อื่นใช้งานผ่านอินเทอร์เน็ต:

Database (ฐานข้อมูล): แนะนำให้สมัครและใช้ MongoDB Atlas (ฟรี) เพื่อเก็บข้อมูลบนคลาวด์ นำ URL ที่ได้ไปตั้งค่าใน .env ของ Backend

Backend (เซิร์ฟเวอร์ Node.js): นำไปฝากไว้ที่บริการอย่าง Render, Railway, หรือ DigitalOcean App Platform (อย่าลืมตั้งค่า Environment Variables ให้ครบ)

Frontend (เว็บไซต์ Next.js): นำไป Host ที่ Vercel (ฟรีและรองรับ Next.js ได้ดีที่สุด)
ข้อควรระวัง: ก่อนนำขึ้น Vercel อย่าลืมเปลี่ยนค่า GAS_API_URL ในโค้ด Frontend ให้เป็น URL ของ Backend ที่ได้จากบริการอย่าง Render หรือ Railway


นี่คือ Workflow (กระแสการทำงานของระบบ) และ รายการฟังก์ชันทั้งหมด (Function Definitions) ที่มีอยู่ในโค้ดปัจจุบัน เพื่อให้คุณสามารถนำไปทำ Document หรือใช้อ้างอิงในการพัฒนาต่อยอด (Refactor / Scale-up) ได้ครับ

🌊 System Workflow (กระแสการทำงานของระบบ)
Authentication Flow (การเข้าสู่ระบบ):

ระบบเริ่มต้นที่หน้าจอ Login (isLoggedIn === false)

ผู้ใช้กรอก Username/Password -> กดเข้าสู่ระบบ -> เรียกใช้ handleLogin

ฟังก์ชันยิง API ไปที่ checkLogin หากสำเร็จ จะเซ็ต State user, isLoggedIn เป็น true

Data Initialization (การโหลดข้อมูลเริ่มต้น):

เมื่อ isLoggedIn เปลี่ยนเป็น true -> useEffect จะทำงานและเรียก loadCoreData()

ระบบจะดึงข้อมูลหลัก 4 ส่วนพร้อมกัน (Promise.all): โครงสร้างหน่วย, กำลังพล, ยอดสูญเสีย, ผู้ใช้งาน

เมื่อได้ข้อมูลมา จะถูกเก็บลง State: rawData, personnelData, casualtyData, usersData

Data Processing & Filtering (การประมวลผลและตัวกรอง):

ผู้ใช้สามารถเลือก Dropdown หรือพิมพ์ค้นหาได้ที่ Global Filter Bar -> เรียก handleFilterChange

เมื่อ State filters หรือ rawData เปลี่ยน -> useMemo จะคำนวณ filteredData ใหม่ทันที

จากนั้น useMemo อีกตัวจะคำนวณ dashboardStats (ยอดรวม, แบ่งตามระดับ, สรุปส่วนภูมิภาค) จากข้อมูลที่ถูกกรองแล้ว

UI Rendering & Charting (การแสดงผลและกราฟ):

กราฟ (Chart.js) จะถูกรีเรนเดอร์ผ่าน useEffect ทุกครั้งที่ dashboardStats เปลี่ยนแปลง

แผนที่ (Leaflet) จะถูกตั้งค่าผ่าน useEffect เมื่อผู้ใช้สลับไปที่เมนู map

ตารางและโครงสร้าง Tree จะแสดงผลโดยใช้ข้อมูลจาก filteredData

CRUD Operations (การจัดการข้อมูล เพิ่ม/ลบ/แก้ไข):

กดปุ่มแก้ไข -> นำข้อมูลแถวนั้นไปใส่ใน State ของฟอร์ม (เช่น setUnitForm) -> เปิด Modal

กดปุ่มบันทึก -> เรียกใช้ handleSave... -> ยิง API แบบ POST -> รอผลลัพธ์ -> ปิด Modal -> เรียก loadCoreData() เพื่อดึงข้อมูลใหม่ทั้งหมดมาอัปเดตหน้าจอ

🛠️ รายการฟังก์ชันทั้งหมดในระบบ (Functions Directory)
1. API & Utility Functions (ฟังก์ชันเชื่อมต่อและตัวช่วย)
fetchAPI(action, data): ฟังก์ชันหลักสำหรับยิง POST Request ไปยัง Backend (Google Apps Script หรือ Node.js) โดยส่ง Action Name และ Payload ไป หาก URL ว่างเปล่าหรือ Error จะสลับไปใช้ runDemoMode

runDemoMode(action, data): จำลองข้อมูล (Mock Data) และ Response คืนมาโดยใช้ setTimeout เพื่อจำลองความหน่วงของ Network

cleanNumber(val): ตัวช่วยสำหรับจัดการตัวเลข ลบลูกน้ำ (Comma) และช่องว่าง แล้วแปลงจาก String เป็น Number (หากไม่ใช่ตัวเลขจะคืนค่าเป็น 0)

2. Lifecycle & Data Fetching (ฟังก์ชันโหลดข้อมูล)
loadCoreData(): รวบรวมคำสั่ง fetchAPI เพื่อดึงข้อมูลตารางหลัก 4 ตาราง (Units, Personnel, Casualties, Users) ในครั้งเดียวด้วย Promise.all

3. Data Transformation (ฟังก์ชันคำนวณและประมวลผลข้อมูล - useMemo)
filteredData: ประมวลผลและกรอง rawData ตามเงื่อนไขของ filters (ค้นหาชื่อ, แผน, ส่วนภูมิภาค, ระดับต่างๆ, ชั่นยศ)

dashboardStats: คำนวณสถิติจาก filteredData เพื่อเอาไปแสดงผลบน Dashboard ประกอบด้วย:

ยอดรวมชั้นยศ (g, n, s, p, total, prevTotal)

สรุปจำนวนหน่วยแยกตามระดับ (c_l0, c_l1, c_l2, ...)

จำนวนหน่วยที่มีการเปลี่ยนแปลง/รอดำเนินการ (updated, pending)

กลุ่มข้อมูลสำหรับกราฟ (plans, struct)

4. Event Handlers (ฟังก์ชันจัดการ Action ของผู้ใช้)
handleLogin(e): ดักจับ Event Submit จากฟอร์ม Login, ดึงค่า Username/Password แล้วเรียก API

handleLogout(): ล้าง State isLoggedIn และ user กลับหน้า Login

switchPage(page): สลับหน้าจอ (Active Page) และจัดการ State ของแอนิเมชัน (isPageAnimating) รวมถึงปิด Sidebar ในมือถือ

handleFilterChange(key, val): อัปเดต Object State ของ filters ตาม Key ที่กำหนด

5. Form Submissions (ฟังก์ชันบันทึกข้อมูลลงฐานข้อมูล)
handleSaveUnit(): ส่งข้อมูล unitForm ไปที่ API เพื่อบันทึก/อัปเดต โครงสร้างหน่วย

handleSavePersonnel(): ส่งข้อมูล personnelForm ไปที่ API เพื่อบันทึกข้อมูลกำลังพล

handleSaveUser(): ส่งข้อมูล userForm ไปที่ API เพื่อจัดการข้อมูลบัญชีผู้ใช้

handleSaveCasualty(): ส่งข้อมูล casualtyForm ไปที่ API เพื่อบันทึกข้อมูลสถานภาพสูญเสีย

6. Tree Builders (ฟังก์ชันสร้างผังโครงสร้าง)
buildTreeData(): นำ filteredData มาวนลูปจัดกลุ่ม (Group By) เป็นโครงสร้าง Object ซ้อนกัน 4 ระดับ (L0 > L1 > L2 > Array of Units) เพื่อนำไปวาดเป็น UI สายการบังคับบัญชา

buildDynamicTreeData(): คล้าย buildTreeData แต่แยกหมวดหมู่ตาม plan (แผนยุทธศาสตร์) ก่อน เพื่อใช้สำหรับหน้า "จัดการโครงสร้าง" (Dynamic Tab)

7. Reusable Sub-Components (ฟังก์ชันแสดงผล UI ย่อย)
StatCard: คอมโพเนนต์แสดงกล่องสถิติด้านบน (มี Title, Value, Icon, สี)

NavItem: ปุ่มเมนูใน Sidebar (รองรับสถานะ Active, Animation, Icon)

ModalWrapper: คอมโพเนนต์ครอบ Modal อเนกประสงค์ มีแอนิเมชันเด้งเข้า-ออก จัดการปุ่มบันทึก/ยกเลิก และ Title Bar ให้อัตโนมัติ

🚀 แนวทางการพัฒนาต่อยอด (Scaling Up)
หากคุณต้องการนำโค้ดนี้ไปต่อยอดเป็นโปรเจกต์ Next.js สเกลใหญ่ ผมแนะนำให้ทำตามสเต็ปนี้ครับ:

Component Splitting (การแยกไฟล์):

ไฟล์ App.jsx ปัจจุบันมีขนาดใหญ่มาก (เกิน 500 บรรทัด) แนะนำให้สร้างโฟลเดอร์ /components แล้วแยก UI เป็นไฟล์ย่อยๆ เช่น:
...
/components/views/MapView.jsx

/components/views/StructureTable.jsx

State Management (ย้าย State ออกจาก App.jsx):

เปลี่ยนจากการส่ง Props ลึกๆ ไปใช้ Context API หรือเครื่องมืออย่าง Zustand หรือ Redux Toolkit สำหรับเก็บ Global State เช่น user, rawData, และ filters

Routing (การเปลี่ยนหน้าจริง):

เปลี่ยน activePage (ที่ใช้ if/else เรนเดอร์) ไปใช้ระบบ Routing ของ Next.js (App Router: /app/dashboard/page.jsx, /app/map/page.jsx) จะช่วยให้ประสิทธิภาพ SEO และการโหลดดีขึ้นมาก

Map Component:

แยกแผนที่ Leaflet ออกมาเป็น Component ต่างหาก และใช้ next/dynamic ในการโหลดพร้อมตั้งค่า ssr: false ป้องกันบัคเรื่อง window is not defined ของ Next.js

หากมีส่วนไหนที่ต้องการให้เขียนโค้ดแยกชิ้นให้ หรือต้องการตั้งค่าระบบ Backend Node.js+MongoDB ที่เคยให้ไปเพิ่มเติม แจ้งได้เลยครับ!
...
// ==========================================
const GAS_API_URL = "https://script.google.com/macros/s/AKfycbx4GfOBrL48C1r_YX6xJ2Ec1X36VMEoV6cYXAPmjfJnI_2oeX6T89g2ANwUgM4rFang/exec";
...
const taskForcesTemplate = [
  ...
  { id: 'thepsatri', name: 'กกล. เทพสตรี', stars: 3, color: '#2ecc71', hq: 'ค่ายเทพสตรีศรีสุนทร จ.นครศรีธรรมราช', hqLat: 8.169, hqLng: 99.638, detailedDesc: 'ป้องกันการกระทำผิดกฎหมายข้ามชาติ สินค้าหนีภาษี และรักษาความมั่นคงร่วมกับประเทศเพื่อนบ้าน' },
...
async function fetchAPI(action, data = {}) {
  if (!GAS_API_URL || GAS_API_URL.trim() === "") return runDemoMode(action, data);
  try {
    const response = await fetch(GAS_API_URL, {
      method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ action, data })
    });
    const result = JSON.parse(await response.text());
    if (result.error && result.error.includes("ไม่รู้จักคำสั่ง")) return runDemoMode(action, data);
    return result;
  } catch (error) {
    return runDemoMode(action, data);
  }
}
...
export default function App() {
  ...
  // 4️⃣ Form States
  const [unitForm, setUnitForm] = useState({ rowIndex: '', plan: '', l0: '', l1: '', l2: '', l3: '', l4: '', g: '', n: '', s: '', p: '', checkList: '' });
  ...
  const handleLogin = async (e) => {
    ...
    if (res.success) { setUser({ name: res.name, role: res.role }); setIsLoggedIn(true); } 
  ...
  if (!isLoggedIn) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-[#050505] overflow-hidden font-['Sarabun']">
        ...
        <div className="absolute bottom-0 right-0 w-[30rem] h-[30rem] bg-emerald-900/20 rounded-full blur-[120px] animate-pulse" style={{animationDelay: '2s'}}></div>
        ...
        <div className="relative z-10 w-full max-w-md p-10 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] shadow-2xl flex flex-col items-center">
          <div className="relative mb-6 group">
            ...
            <img src="https://upload.wikimedia.org/wikipedia/commons/e/eb/Emblem_of_the_Royal_Thai_Army.svg" className="w-28 relative z-10 drop-shadow-[0_0_15px_rgba(234,179,8,0.4)]" alt="RTA Logo" />
          </div>
  ...
  // ==========================================
  return (
    <div className="flex h-screen bg-[#070a13] text-slate-200 font-['Sarabun'] overflow-hidden selection:bg-yellow-500/30">
      ...
      <aside className={`fixed lg:relative z-50 h-full bg-[#0b0f19] border-r border-white/5 flex flex-col transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'w-24' : 'w-72'} ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-6 flex flex-col items-center justify-center shrink-0">
          <div className="relative">
             ...
             <img src="https://upload.wikimedia.org/wikipedia/commons/e/eb/Emblem_of_the_Royal_Thai_Army.svg" className={`relative z-10 transition-all duration-300 drop-shadow-md ${sidebarCollapsed ? 'w-10' : 'w-16'}`} alt="logo" />
          </div>
        ...
        <div className="p-4 border-t border-white/5 shrink-0 animate-menu-slide-in" style={{animationDelay: '700ms', opacity: 0}}>
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-colors group">
                                                  ...
                                                  return (
                                                    <div key={u.index} className="flex justify-between items-center bg-black/20 p-3 rounded-xl border border-white/5 hover:border-white/20 transition-colors">
                                                      ...
                                                      <div className="flex items-center gap-3">
                                                         ...
                                                         <button onClick={() => { setUnitForm({ rowIndex: u.data[18], plan: u.data[0], l0: u.data[1], l1: u.data[2], l2: u.data[3], l3: u.data[4], l4: u.data[5], g: u.data[6], n: u.data[7], s: u.data[8], p: u.data[9], checkList: u.data[11] }); setShowUnitModal(true); }} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-slate-400 hover:text-white transition-colors"><i className="bi bi-pencil-square"></i></button>
                                                      </div>
                                                    </div>
                                                  )
                                               })}
